...
class Installer
{
    ...
    private function registerConfigurationFile($configFile, $type = 'config')
    {
        if (file_exists($configFile)) {
             ...
             */
            $symfonyRoot = ParserUtil::strReplaceLast(
                'app',
                '',
    ...
    private function registerModule($params)
    {
        ...
        if (is_array($params['modules']) && count($params['modules'])) {
            foreach ($params['modules'] as $identifier => $moduleParams) {
                ...
                if (isset($moduleParams['hooks']) && is_array($moduleParams['hooks']) && count(
                        $moduleParams['hooks']
                    )
                ) {
                ...
                if (isset($moduleParams['system']) && is_array($moduleParams['system']) && count(
                        $moduleParams['system']
                    )
                ) {
                ...
                if ($this->newBundle->getType(
                    ) == 'campaignchain-operation' && !isset($moduleParams['params']['owns_location'])
                ) {
                    throw new \Exception(
                        "You must set the 'owns_location' parameter in campaignchain.yml to 'true' or 'false' for module '".$identifier."' in bundle '".$this->newBundle->getName(
                        )."'."
                    );
                }
                ...
                if (
                    ...
                    $this->campaignConversions[$this->newBundle->getName()][$module->getIdentifier(
                    )] = $moduleParams['conversions'];
                }
                ...
                if (
                    ...
                    $this->activityChannels[$this->newBundle->getName()][$module->getIdentifier(
                    )] = $moduleParams['channels'];
                }
            }
        }
    }
