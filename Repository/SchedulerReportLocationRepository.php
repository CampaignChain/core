...
class SchedulerReportLocationRepository extends EntityRepository
{
    /**
     * @param \DateTime $periodStart
     * @param \DateTime $periodEnd
     * @return SchedulerReportLocation[]
     */
    public function getScheduledReportJobsForSchedulerCommand(\DateTime $periodStart, \DateTime $periodEnd)
    {
        return $this->createQueryBuilder('sr')
            ...
            ->andWhere(
                "NOT EXISTS (SELECT j.id FROM CampaignChain\CoreBundle\Entity\Job j WHERE j.status = :jobStatus AND sr.location = j.actionId AND j.actionType = :reportType)"
            )
            ->setParameter('reportType', 'location')
